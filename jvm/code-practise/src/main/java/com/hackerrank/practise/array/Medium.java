...
public class Medium {
    ...
    public static int[] topKFrequent(int[] nums, int k) {
        ...
        Comparator<Map.Entry<Integer, Integer>> comparator = (o1, o2) -> o2.getValue().compareTo(o1.getValue());
        // sort the map in decreasing value order
        int[] sorted = map.entrySet().stream().sorted(comparator).limit(k + 1).map(Map.Entry::getKey).mapToInt(i -> i).toArray();
    ...
    public int longestConsecutive(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            // check forward
            while (map.containsKey(val) && map.get(val) == false) {
